 ...
 * @see Walker
 */
class Sky_Walker_Nav_Menu extends Walker {
	/**
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 The {@see 'skywp_nav_menu_item_args'} filter was added.
	 *
	 * @see Walker::start_el()
	 *
	 * @param string   $output Used to append additional content (passed by reference).
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		 ...
		 * @param int      $depth   Depth of menu item. Used for padding.
		 */
		$class_names = join( ' ', apply_filters( 'skywp_nav_menu_css_class', array_filter( $classes ), $item, $args, $depth ) );
		$class_names = $class_names ? ' class="' . esc_attr( $class_names ) . '"' : '';
		 ...
		 * @param int      $depth   Depth of menu item. Used for padding.
		 */
		$id = apply_filters( 'skywp_nav_menu_item_id', 'menu-item-' . $item->ID, $item, $args, $depth );
 ...
 */
add_filter( 'skywp_walker_nav_menu_start_el', 'skywp_arrows_in_menus', 10, 4 );
function skywp_arrows_in_menus( $item_output, $item, $depth, $args ) {
