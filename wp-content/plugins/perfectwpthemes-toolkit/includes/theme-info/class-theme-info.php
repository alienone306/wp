 ...
 * @subpackage Perfectwpthemes_Toolkit/includes/common/theme-info
 */

if ( ! class_exists( 'Perfectwpthemes_Toolkit_Theme_Info' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Perfectwpthemes_Toolkit_Theme_Info {
		...
		/**
		 * Singleton instance of Perfectwpthemes_Toolkit_Theme_Info.
		 *
		 * @var Perfectwpthemes_Toolkit_Theme_Info $instance Perfectwpthemes_Toolkit_Theme_Info instance.
		 */
		private static $instance;

		/**
		 * Main Perfectwpthemes_Toolkit_Theme_Info instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Perfectwpthemes_Toolkit_Theme_Info ) ) {
				self::$instance = new Perfectwpthemes_Toolkit_Theme_Info;
		...
		public function setup_config() {
			...
			$this->logo_url      = isset( $this->config['logo_url'] ) ? $this->config['logo_url'] : plugin_dir_url( __FILE__ ) . 'images/perfectwpthemes.png';
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://www.perfectwpthemes.com/';
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'perfectwpthemes-toolkit' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'perfectwpthemes-toolkit' ), $this->theme_name ) . '</a></p>' );
		}
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'perfectwpthemes-toolkit-theme-info', plugin_dir_url( __FILE__ ) . 'css/theme-info.css' );
				wp_enqueue_script( 'perfectwpthemes-toolkit-theme-info', plugin_dir_url( __FILE__ ) . 'js/theme-info.js', array( 'jquery' ), PERFECTWPTHEMES_TOOLKIT_VERSION );

				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'perfectwpthemes-toolkit-about', 'ttkitAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'perfectwpthemes-toolkit' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'perfectwpthemes-toolkit' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'perfectwpthemes-toolkit' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'perfectwpthemes-toolkit' );
