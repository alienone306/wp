    ...
    public function setTasks($tasks) {
        ...
        $this->_tasks[$this->_category] = array(
            'optimization' => array(
                ...
                'description' => sprintf(__("Make sure this Focus Page is optimized to 100%% using the Squirrly SEO Live Assistant. %s As you can see clearly on Google search result pages, Googles tries to find the closest match (inside web content) to what the user searched for. %s That is why using this method of optimizing a page as outlined by the Live Assistant feature is mandatory. %s Don't worry about over-optimizing anything, as the Live Assistant checks for many over-optimization traps you may fall into.", _SQ_PLUGIN_NAME_), '<br /><br />', '<br /><br />', '<br /><br />'),
            ),
            'snippet' => array(
                'title' => __("Snippet is green", _SQ_PLUGIN_NAME_),
                'description' => sprintf(__("The tasks inside the Snippet section of the Focus Pages feature must all be turned green. %s Why? %s If the Snippet elements are Not green, then your Focus Page is not 100%% optimized. %s We've built this SEO Content section especially because we wanted to help you understand that there's a lot more to On-Page SEO than just a content analysis, or a snippet. You need all these elements working together in order to achieve high rankings.", _SQ_PLUGIN_NAME_), '<br /><br />', '<br /><br />', '<br /><br />'),
            ),
            'onpageseo' => array(
    ...
    public function getHeader() {
        ...
        if ($this->_keyword) {
            ...
            if ((int)$this->_post->ID > 0) {
                $header .= '<a href="' . $edit_link . '" target="_blank" class="btn bg-primary text-white col-sm-8 offset-2 mt-3">' . __('Optimize for this', _SQ_PLUGIN_NAME_) . '</a>';
            }
        } else {
