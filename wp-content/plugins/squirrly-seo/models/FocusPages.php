...
class SQ_Models_FocusPages {
    ...
    public function init() {
        ...
        $this->_task_categories = array(
            'indexability' => __('Visibility', _SQ_PLUGIN_NAME_),
            'keyword' => __('Keyword', _SQ_PLUGIN_NAME_),
            'strategy' => __('Strategy', _SQ_PLUGIN_NAME_),
            'content' => __('SEO Content', _SQ_PLUGIN_NAME_),
            'length' => __('Words / Page', _SQ_PLUGIN_NAME_),
            'onpage' => __('Platform SEO', _SQ_PLUGIN_NAME_),
            'snippet' => __('Snippet', _SQ_PLUGIN_NAME_),
            'image' => __('SEO Image', _SQ_PLUGIN_NAME_),
            'traffic' => __('Traffic Health', _SQ_PLUGIN_NAME_),
            'audit' => __('Platform Health', _SQ_PLUGIN_NAME_),
            'authority' => __('Page Authority', _SQ_PLUGIN_NAME_),
            'social' => __('Social Signals', _SQ_PLUGIN_NAME_),
            'backlinks' => __('Backlinks', _SQ_PLUGIN_NAME_),
            'innerlinks' => __('Inner Links', _SQ_PLUGIN_NAME_),
            'nofollow' => __('Outbound Links', _SQ_PLUGIN_NAME_),
            'accuracy' => __('Accuracy', _SQ_PLUGIN_NAME_),
            'ctr' => __('CTR', _SQ_PLUGIN_NAME_),
            'impressions' => __('Impressions', _SQ_PLUGIN_NAME_),
            'clicks' => __('Clicks', _SQ_PLUGIN_NAME_),
        );
        ...
        foreach ($this->_task_categories as $category => $array) {
            ...
            if (isset($this->_assistant_categories[$this->_focuspage->id][$category])) {
                if (!$this->_assistant_categories[$this->_focuspage->id][$category]['completed'] && !$this->_assistant_categories[$this->_focuspage->id][$category]['error']) {
                    ...
                    //substract how much will affect the visibility this category
                    $this->_focuspage->visibility -= (int)$this->_assistant_categories[$this->_focuspage->id][$category]['penalty'];

                    //increase incomplete categories
                    $this->_focuspage->incomplete += 1;
                    $this->_task_categories_labels[$category]['show'] = true;

                } else {
                    if (is_array($labels) && !empty($labels)) {
                        if (in_array($category, $labels)) {
                            $this->_focuspage = false;
                            return $this;
                        }
                    }
                }
    ...
    public function getCategories() {
        return json_decode(json_encode($this->_task_categories));
    }

    public function getAssistantCategories($id) {
        //SQ_Debug::dump($id,array_keys($this->_assistant_categories));
        return json_decode(json_encode($this->_assistant_categories[$id]));
    }

    public function getLabels() {
        return json_decode(json_encode(apply_filters('sq_categories_labels', $this->_task_categories_labels)));
    }
